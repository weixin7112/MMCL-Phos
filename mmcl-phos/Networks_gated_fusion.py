...
class GatedFusion(nn.Module):
    ...
    def __init__(self, dim_b, dim_c, dim_d, output_dim, hidden_dim=128):
        super().__init__()
        self.trans_b = nn.Linear(dim_b, output_dim)
        self.trans_c = nn.Linear(dim_c, output_dim)
        self.trans_d = nn.Linear(dim_d, output_dim)
    ...
    def forward(self, feat_b, feat_c, feat_d):
        h_b = torch.tanh(self.trans_b(feat_b))
        h_c = torch.tanh(self.trans_c(feat_c))
        h_d = torch.tanh(self.trans_d(feat_d))

        concat = torch.cat([feat_b, feat_c, feat_d], dim=1)
        gates = self.gate(concat)                      # (B, output_dim * 3)
        g_b, g_c, g_d = torch.chunk(gates, 3, dim=1)   # each (B, output_dim)

        fused = g_b * h_b + g_c * h_c + g_d * h_d
        return fused
